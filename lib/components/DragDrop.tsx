...
interface DragDropState extends ContainerState {
  items: any[];
  zones: any[];
  dragging: boolean;
...
interface DragDropProps extends ContainerProps {
  items: any[];
  zones: any[];
...
const PERCENT = 0.15;
class DragDrop extends Container<DragDropProps, DragDropState> {
  state: DragDropState = {
  ...
  timeout: NodeJS.Timeout | string | number | null = null;
  ref = React.createRef<ScrollView>();

  componentDidUpdate(nextProps: DragDropProps) {
